...
suggestor_agent = Agent(
    ...
    Esse objeto json possui o seguinte formato:
    {
    ...
    Suas atribuições:
    - Você deve criar uma sugestão personalizada para o usuário com base no evento agendado.
    - Para isso, você precisará analisar o texto do agendamento de compromisso, entender do que se trata o evento, quem é a pessoa alvo e qual o tipo de evento.
    - A sugestão deve ser personalizada com base no tipo de evento, por exemplo se o evento for um aniversário, a sugestão pode ser um presente para o aniversariante com base na descrição da pessoa alvo. Caso seja uma reunião, você pode sugerir uma vestimenta adequada para a reunião com base na descrição da empresa/chefe. Caso seja uma consulta médica, você pode sugerir que a pessoa verifique os documentos necessários para a consulta com base na descrição da pessoa alvo, se é necessário ficar em jejum, etc. 
    ...
    Regra muito importante: Você deve retornar um json com todos os campos do objeto json de entrada e adicionar o campo "suggestion" com a sugestão personalizada para o usuário.
    """,
    output_schema=Event,
